...
def take_off(a_target_altitude, manualControl):
    ...
    if manualControl:
        ...
        w.start()
        s = threading.Thread(target=bear)
        s.start()


def prepare_command_vel(velocity_x, velocity_y, velocity_z):
    """
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        mavutil.mavlink.MAV_FRAME_LOCAL_NED,  # frame - relative to heading //CHANGED
...
def prepare_command_vel_rel(velocity_x, velocity_y, velocity_z):
    """
        Move vehicle in direction based on specified velocity vectors.
        """
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
...
def prepare_command_heading(heading, relative, clockwise):
    if relative:
...
def flying():
    ...
    cmd = prepare_command_vel(0, 0, 0)  # stop
    while not end:
        go = False
        while not go:
        ...
        if direction == "North":
            cmd = prepare_command_vel(speed, 0, 0)  # NORTH
        if direction == "South":
            cmd = prepare_command_vel(-speed, 0, 0)  # SOUTH
        if direction == "East":
            cmd = prepare_command_vel(0, speed, 0)  # EAST
        if direction == "West":
            cmd = prepare_command_vel(0, -speed, 0)  # WEST
        if direction == "NorthWest":
            cmd = prepare_command_vel(speed, -speed, 0)  # NORTHWEST
        if direction == "NorthEst":  # Cambiado (typo en NorthEast, ponia NorthEst)
            cmd = prepare_command_vel(speed, speed, 0)  # NORTHEST
        if direction == "SouthWest":
            cmd = prepare_command_vel(-speed, -speed, 0)  # SOUTHWEST
        if direction == "SouthEst":  # Cambiado (typo en SouthEast, ponia SouthEst)
            cmd = prepare_command_vel(-speed, speed, 0)  # SOUTHEAST
        if direction == "Stop":
            cmd = prepare_command_vel(0, 0, 0)  # STOP
        if direction == "Front":
            cmd = prepare_command_vel_rel(speed, 0, 0)
        if direction == "Back":
            cmd = prepare_command_vel_rel(-speed, 0, 0)
        if direction == "RTL":
            end = True
...
def bear():
    ...
    global end
    degrees = 20
    cmd = prepare_command_heading(0, True, 1)
    while not end:
        bear = False
        while not bear:
            vehicle.send_mavlink(cmd)
        ...
        if rotation == "Clockwise":
            cmd = prepare_command_heading(degrees, True, 1)
...
def process_message(message, client):
    ...
    if command == "bear":
        rotation = message.payload.decode("utf-8")
        print("Bearing ", rotation)
        bear = True
...
def AutopilotService(connection_mode, operation_mode, external_broker, username, password):
    ...
    external_client.loop_forever()  # Cambiado
